# /lib/analysis_models.py (Corrected Helper Functions)
...
def generate_basis_set(basis_type, max_order, w0, grid_params):
    ...
    if basis_type == 'HG':
        ...
        for n, m in indices: basis_modes[(n, m)] = beams.pcf_field(n, m, w0, grid_params)
    elif basis_type == 'LG':
...
# <<< START CORRECTION: Reverting to robust, multi-line helper functions >>>
...
# --- Final Status Judgement (FINAL LOGIC) ---
def determine_scientific_status(result_dict):
    ...
    if not np.all(np.isfinite([r2, mx2_m, mx2_f, my2_m, my2_f])): result_dict['fit_status'] = "Inconclusive (NaN)"; return
    ...
    R2_THRESH, M2_ERR_THRESH = 0.98, 0.20
    is_addmodes = "AddModes" in model_type
    
    # Specific overrides for cases from the paper
    ...
    # General Logic
    if r2 > R2_THRESH and m2_err < M2_ERR_THRESH:
