# /generate_table_1.py (Complete and Final, with Ring Beam)
...
def main():
    ...
    job_definitions = [
        ...
        {'beam_key': 'SG(N=10) Defocus S',   'model_type': 'AddModes(LG, M=20)',     'grid_size': 256, 'psi_ref_func': beams.get_sg_defocus_strong, 'basis_type': 'LG', 'max_order': 20, 'basis_w0': 5.0, 'm2_method': 'coeff_simple'},
        ...
        {'beam_key': 'MultiMode',            'model_type': 'AddModes(HG, M=4, Robust)', 'grid_size': 256, 'psi_ref_func': beams.get_multimode, 'basis_type': 'HG', 'max_order': 4, 'basis_w0': 1.0, 'm2_method': 'coeff_robust'},
        ...
        {'beam_key': 'Ring Beam',            'model_type': 'MultPoly(Gauss Base)',   'grid_size': 256, 'psi_ref_func': beams.get_ring_beam, 'base_func': beams.get_tem00_ideal},
        {'beam_key': 'Ring Beam',            'model_type': 'AddModes(LG, M=6)',      'grid_size': 256, 'psi_ref_func': beams.get_ring_beam, 'basis_type': 'LG', 'max_order': 6, 'basis_w0': 1.5, 'm2_method': 'coeff_simple'},
        {'beam_key': 'Ring Beam',            'model_type': 'AddModes(HG, M=6, Robust)', 'grid_size': 256, 'psi_ref_func': beams.get_ring_beam, 'basis_type': 'HG', 'max_order': 6, 'basis_w0': 1.5, 'm2_method': 'coeff_robust'},
    ]
...
def print_results_table(results):
    ...
    order_map = [
        ...
        ('SG(N=10) Defocus S', 'AddModes(LG, M=20)'),
        ('SG(N=10) Defocus S', 'MultPoly(Defocus SG Base)'), 
        ('TEM00 Ab Strong', 'MultPoly(Gauss Base)'),
        ('AGauss', 'AddPoly(Gauss Base)'), ('NP', 'AddPoly(Gauss Base)'), ('NL', 'AddGauss(Gauss Base)'),
        ('MultiMode', 'AddModes(HG, M=4)'),
        ('MultiMode', 'AddModes(HG, M=4, Robust)'),
        ('TEM00 Ab Strong', 'AddModes(LG, M=16)'), ('Ince-Gaussian', 'AddModes(LG)'),
        ...
        ('Ring Beam', 'MultPoly(Gauss Base)'),
        ('Ring Beam', 'AddModes(LG, M=6)'),
        ('Ring Beam', 'AddModes(HG, M=6, Robust)'),
    ]
    ...
    for beam_key, model_key in order_map:
        ...
        if 'AddModes' in model_key:
            m2_method = res.get('job', {}).get('m2_method', 'coeff_simple')
            if m2_method == 'coeff_robust':
                m2f_str += " (C,R)"
            else:
                m2f_str += " (C)"
        else:
