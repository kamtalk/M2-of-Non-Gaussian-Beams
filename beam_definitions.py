# /lib/beam_definitions.py (UPDATED with Cosh-Gaussian)
...
# --- Core Field Calculation Functions (Unchanged) ---
def pcf_field(n, m, w0, grid_params):
    b = np.sqrt(2) / max(w0, 1e-9)
    x_in = (b * grid_params['xx']).astype(np.float64)
    y_in = (b * grid_params['yy']).astype(np.float64)
    v_in_x, v_in_y = float(n), float(m)
    try:
        dv_x, _ = pbdv(v_in_x, x_in)
        dv_y, _ = pbdv(v_in_y, y_in)
        field = dv_x * dv_y
        field[~np.isfinite(field)] = 0.0
        return field.astype(np.complex128)
    except Exception as e:
        logger.error(f"Error in pbdv for HG({n},{m}): {e}")
        return np.zeros_like(grid_params['xx'], dtype=np.complex128)
...
def get_tem13_ideal(grid_params): return pcf_field(1, 3, TEM13_W0, grid_params)
...
def get_multimode(grid_params):
    ...
    for (n,m), c in coeffs.items(): psi += c * pcf_field(n, m, 1.0, grid_params)
    return psi
def get_ince_gaussian(grid_params):
    ...
    try:
        Pmn_eta, _ = lpmn(IG_M, IG_P, np.cos(eta)); Pmn_xi, _ = lpmn(IG_M, IG_P, np.tanh(xi))
        psi = gauss_env * Pmn_xi[IG_M, IG_P, :, :] * Pmn_eta[IG_M, IG_P, :, :]; psi[~np.isfinite(psi)] = 0.0
        power = np.sum(np.abs(psi)**2);
        if power > 1e-12: psi /= np.sqrt(power)
        return psi.astype(np.complex128)
    except Exception: return np.zeros_like(grid_params['xx'], dtype=np.complex128)
...
def get_mathieu_beam(grid_params):
    ...
    x_prime = xx * (1 + e); y_prime = yy * (1 - e)
    psi = (2 * x_prime / w0) * (2 * y_prime / w0) * np.exp(-(x_prime**2 + y_prime**2) / w0**2)
    power = np.sum(np.abs(psi)**2);
    if power > 1e-12: psi /= np.sqrt(power)
    return psi.astype(np.complex128)
...
def get_cosh_gaussian_beam(grid_params):
    ...
    # A 2D Cosh-Gaussian beam, separable in x and y
